...
import { ArrowUp, TrendingUp, X, ChevronsDownUp, Move } from 'lucide-react'
...
export const FloatingDiscoveryMenu = () => {
  ...
  useEffect(() => {
    if (sessionStorage.getItem('discovery-menu-closed') === 'true') {
      setIsClosed(true);
    }
  }, [])
  ...
  const handleClose = () => {
    sessionStorage.setItem('discovery-menu-closed', 'true');
    setIsClosed(true);
  }
  ...
  if (isMinimized) {
    return (
      <div className="fixed bottom-6 right-6 z-50">
        <Button onClick={() => setIsMinimized(false)} className="rounded-full w-16 h-16 shadow-lg">
  ...
  return (
    <Draggable nodeRef={nodeRef} handle=".drag-handle">
      <div ref={nodeRef} className="fixed bottom-6 right-6 z-50 space-y-2 transition-all duration-300 cursor-grab">
        <Card className="w-80 shadow-2xl border-2 border-primary/20 bg-background/80 backdrop-blur-sm">
          <CardHeader className="p-4 flex flex-row items-center justify-between drag-handle">
          ...
          <CardContent className="p-0 cursor-auto">
            <Tabs defaultValue="community" className="w-full">
              ...
              <TabsContent value="community" className="p-4 space-y-2 h-48 overflow-y-auto">
                {loading ? (
              ...
              <TabsContent value="news" className="p-4 space-y-2 h-48 overflow-y-auto">
                {loading ? (
                  <p className="text-sm text-muted-foreground">불러오는 중...</p>
                ) : (
        ...
        <Button
          size="icon"
          className="rounded-full shadow-lg w-12 h-12 ml-auto block cursor-pointer"
          onClick={scrollToTop}
          aria-label="맨 위로 스크롤"
        >
